...
export function OngoingOrdersTable() {
  ...
  const fetchOrders = async () => {
    try {
      setLoading(true);
      setError(null);
      const response = await fetch("/api/gestionnaire/commandes-en-attente");
      if (!response.ok) {
  ...
  const updateOrderStatus = async (orderId: string, newStatus: "ANNULEE") => {
    try {
      setProcessingOrder(orderId);
      const formData = new FormData();
      formData.append("statut", newStatus);
      const response = await fetch(`/api/gestionnaire/commandes-en-attente/${orderId}/status`, {
        method: "PUT",
        body: formData,
      });
      if (!response.ok) {
  ...
  const handleCancelOrder = () => {
    ...
    updateOrderStatus(orderToCancel, "ANNULEE");
  ...
  return (
    <div className="p-6 bg-white rounded-xl shadow-lg border border-gray-100">
      ...
      >
        ...
        >
          <DialogContent className="sm:max-w-[600px] bg-white rounded-xl shadow-2xl border border-red-100">
            <button
            ...
            <DialogHeader>
              <DialogTitle className="text-xl font-bold text-gray-800 bg-clip-text text-transparent bg-gradient-to-r from-red-600 to-red-700">
