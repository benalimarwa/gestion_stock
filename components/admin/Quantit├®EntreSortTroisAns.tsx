...
interface OrderStatsData {
  ...
  approved: number; // Quantité Sortie
  delivered: number; // Quantité Entrée
}
...
export default function QuantitéEntreSortTroisAns() {
    ...
    const fetchProducts = async () => {
      try {
        ...
        if (!response.ok) {
          ...
          throw new Error(errorData.error || 'Échec du chargement des produits');
        }
        ...
        setError(`Erreur lors du chargement des produits: ${err instanceof Error ? err.message : String(err)}`);
        console.error(err);
      }
    };
  ...
  useEffect(() => {
    ...
    const fetchOrderStats = async () => {
      ...
      setError(null);
      try {
        const response = await fetch(`/api/admin/dashboard/qte-entre-sort-trois-annee?productId=${selectedProduct}`);
        if (!response.ok) {
          const errorData = await response.json();
          throw new Error(`${errorData.error || 'Échec de la récupération des statistiques'}${errorData.details ? `: ${errorData.details}` : ''}`);
        }
        const data: OrderStatsData[] = await response.json();
        console.log('Données du graphique récupérées:', data);
        setChartData(data);
      } catch (err) {
  ...
  const totalSortie = chartData.reduce((sum, item) => sum + item.approved, 0);
  const totalEntree = chartData.reduce((sum, item) => sum + item.delivered, 0);
  ...
  return (
    <Card className="w-full overflow-hidden border-none shadow-lg">
        ...
        <div className="flex justify-between items-center">
          <div>
            <CardTitle className="text-2xl font-bold text-slate-800 dark:text-white">
              Statistiques des Stocks (3 Ans)
            </CardTitle>
            <CardDescription className="mt-1 text-slate-600 dark:text-slate-300">
              Suivi des quantités entrées et sorties sur les trois dernières années
            </CardDescription>
          </div>
      ...
      <CardFooter className="flex-col items-start gap-4 pt-2 pb-6 px-6 bg-slate-50 dark:bg-slate-800/50">
        <h4 className="text-sm font-medium text-slate-700 dark:text-slate-300">Résumé sur 3 ans</h4>
        <div className="grid grid-cols-2 w-full gap-6">
          <div className="flex items-center gap-3 bg-white dark:bg-slate-800 p-4 rounded-xl shadow-sm">
            ...
            <div className="flex flex-col">
              <span className="text-xs text-slate-500 dark:text-slate-400">Quantité Sortie</span>
              <span className="font-semibold text-lg text-slate-800 dark:text-white">{totalSortie}</span>
            </div>
          </div>
          <div className="flex items-center gap-3 bg-white dark:bg-slate-800 p-4 rounded-xl shadow-sm">
            ...
            <div className="flex flex-col">
              <span className="text-xs text-slate-500 dark:text-slate-400">Quantité Entrée</span>
              <span className="font-semibold text-lg text-slate-800 dark:text-white">{totalEntree}</span>
            </div>
          </div>
        </div>
      </CardFooter>
