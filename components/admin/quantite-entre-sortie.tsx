...
export function QuantitéEntreSort() {
  ...
  useEffect(() => {
    ...
    const fetchOrderStats = async () => {
      ...
      setError(null);
      try {
        const response = await fetch(`/api/admin/dashboard/qte-prod-entre-sort?productId=${selectedProduct}`);
        if (!response.ok) {
          const errorData = await response.json();
        ...
        <div className="flex justify-between items-center">
          <div>
            ...
            <CardDescription className="mt-1 text-slate-600 dark:text-slate-300">
              Suivi des quantités entrées et sorties de Janvier   à Decembre 
            </CardDescription>
          </div>
