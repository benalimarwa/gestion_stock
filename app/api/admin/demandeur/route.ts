...
export async function POST(request: NextRequest) {
  try {
    ...
    const clerkUser = await clerk.users.createUser({
      ...
      if (typeof err === 'object' && err !== null && 'clerkError' in err && 'errors' in err) {
        const clerkErr = err as { errors: Array<{ message: string }> };
        throw new Error(`Erreur Clerk: ${clerkErr.errors[0].message}`);
      }
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    if (demandeur.user.clerkUserId) {
      await clerk.users.deleteUser(demandeur.user.clerkUserId).catch((err: unknown) => {
