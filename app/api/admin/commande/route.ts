...
export async function POST(request: Request) {
  try {
    ...
    const { produits, statut = "VALIDE" } = body;
    ...
    // Find a supplier that provides all selected products
    const fournisseur = await prisma.fournisseur.findFirst({
      ...
      orderBy: {
        score: 'desc', // Optional: Select supplier with highest score
      },
    });

    if (!fournisseur) {
      return NextResponse.json(
        { error: "Aucun fournisseur ne fournit tous les produits sélectionnés" },
        { status: 400 }
      );
    }
    ...
    const commande = await prisma.commande.create({
      data: {
        ...
        date: new Date(),
        datePrevu: new Date(), // Default to current date
        produits: {
