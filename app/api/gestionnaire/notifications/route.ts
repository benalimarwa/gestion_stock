...
// Define a custom type to avoid conflict with browser's Notification
...
export async function GET() {
  try {
    ...
    // 1. Créer des notifications pour les produits en rupture ou critique
    ...
    for (const produit of produitsCritiques) {
      ...
      // Create an alert (as per existing logic)
      ...
      // Create a notification in the Notification table for the admin
    ...
    // 2. Créer des notifications pour les demandes en attente
    ...
    for (const demande of demandesEnAttente) {
      ...
      const description = `Demande en attente de ${demandeurName} pour: ${produitsList} (ID: ${demande.id})`;
    ...
    // 3. Créer des notifications pour les commandes en cours
    ...
    // 4. Créer des notifications pour les commandes validées
    ...
    for (const commande of commandesValidees) {
      const produitsList = commande.produits.map(cp => `${cp.produit.nom} (${cp.quantite})`).join(", ");
      const description = `Commande validée (ID: ${commande.id}) auprès de ${commande.fournisseur.nom}: ${produitsList}`;
      ...
      if (admin) {
        ...
        if (!notificationExistante) {
          ...
          console.log(`Notification créée pour commande validée: ${commande.id}`);
        }
      }
    }

    // 5. Créer des notifications pour les demandes exceptionnelles acceptées
    const demandesExceptionnellesAcceptees = await prisma.demandeExceptionnelle.findMany({
      where: { statut: "ACCEPTEE" },
      include: {
        produitsExceptionnels: { include: { produitExceptionnel: true } },
        demandeur: { include: { user: true } },
      },
    });

    for (const demande of demandesExceptionnellesAcceptees) {
      const produitsList = demande.produitsExceptionnels.map(dp => `${dp.produitExceptionnel.name} (${dp.quantite})`).join(", ");
      const demandeurName = demande.demandeur.user.name || "Utilisateur";
      const description = `Demande exceptionnelle acceptée de ${demandeurName} pour: ${produitsList} (ID: ${demande.id})`;
      ...
      if (admin) {
        ...
        if (!notificationExistante) {
          ...
          console.log(`Notification créée pour demande exceptionnelle acceptée: ${demande.id}`);
        }
      }
    }

    // 6. Récupérer toutes les notifications existantes
    const notifications = await prisma.notification.findMany({
      orderBy: { dateEnvoi: "desc" },
      include: { utilisateur: true },
    });

    const mappedNotifications: CustomNotification[] = notifications.map((notification) => {
      ...
      if (notification.message.includes("Demande en attente")) {
        ...
        if (match) demandeId = match[1];
      } else if (notification.message.includes("Demande exceptionnelle acceptée")) {
        const match = notification.message.match(/Demande exceptionnelle acceptée de .+ pour: .+ \(ID: (.+)\)/);
        if (match) demandeId = match[1];
      }
    ...
    console.log("Notifications:", mappedNotifications);
...
// ... (DELETE and PATCH handlers remain unchanged) ...
